...
function booklet(target, options, id){
	...
	resetCSS = function(){
		...
		if(opts.closed && opts.autoCenter && opts.curr >= opts.pTotal-2){
			p2.hide();
		}
	...
	gotoPage = function(num){
		...
		if(num > opts.curr && num < opts.pTotal && num >= 0 && !busy){
			...
			p3.stop().animate({left:opts.pWidthH, width:opts.pWidthH, paddingLeft: opts.shadowBtmWidth}, opts.speedH, opts.easeIn)
					 .animate({left:0, width:opts.pWidth, paddingLeft:0}, opts.speedH, opts.easeOut);			
	...
	initOptions = function(){
		...
		//add corner-peels
		if(opts.peels){
			peelP = $('<div class="b-peel b-peel-prev" title="Previous Page"><img src="images/peel-prev.png" alt="" /></div>').appendTo(ctrls);
			peelN = $('<div class="b-peel b-peel-next" title="Next Page"><img src="images/peel-next.png" alt="" /></div>').appendTo(ctrls);
			peels = b.find('.b-peel');
			peel_IMGs = b.find('.b-peel img').width(0);
			peelP_IMG = b.find('.b-peel-prev img');
			peelN_IMG = b.find('.b-peel-next img');
			...
			peelN_IMG.bind('mousedown touchstart MozTouchDown', function(e){
				...
				$(this).bind('mousemove touchmove MozTouchMove', function(e){
					...
					if(origPos != ''){
						var newPos = e.pageX;
						var newSize = opts.peelSize + (1.5*opts.peelSize) * (origPos - newPos)/100;
						$(this).width(newSize);
						$(this).height(newSize);
			...
			peelP_IMG.bind('mousedown touchstart MozTouchDown', function(e){
				...
				$(this).bind('mousemove touchmove MozTouchMove', function(e){
					...
					if(origPos != ''){
						var newPos = e.pageX;
						var newSize = opts.peelSize + (1.5*opts.peelSize) * (newPos - origPos)/100;
						$(this).width(newSize);
						$(this).height(newSize);
						$(this).parent().width(newSize);
						$(this).parent().height(newSize);
			...
			$(b).bind('mouseenter',function(e){
				$(this).bind('mousemove touchmove MozTouchMove',function(e){
					...
					if(!busy){
						...
						if(relativeX < opts.peelSize && (opts.closed && opts.curr != 0)){
							hoverAnimStart(false);
							$(b).bind('click',function(e){e.preventDefault(); prev();});
						}else if(relativeX > opts.pWidth-opts.pagePadding && opts.curr == 0 && opts.autoCenter && opts.closed){
							hoverAnimStart(true);
							$(b).bind('click',function(e){e.preventDefault(); next();});
						}else if(relativeX > opts.pagePadding && relativeX < opts.width-opts.pagePadding){
							hoverAnimEnd(false);
							hoverAnimEnd(true);
							$(b).unbind('click');
						}else if(relativeX > opts.width-opts.pagePadding){
							hoverAnimStart(true);
							$(b).bind('click',function(e){e.preventDefault(); next();});
						}
					}
	...
	hoverAnimStart = function(inc){
		if(!busy){
			$(b).css('cursor',opts.cursor);
			if(inc && !rhover){
				peelN.stop().animate({'width':opts.peelSize,'height':opts.peelSize}, 200, "easeOutExpo");
				peelN_IMG.stop().animate({'width':opts.peelSize,'height':opts.peelSize}, 200, "easeOutExpo");
				rhover = true;
			}else if(!inc && !lhover){
				peelP.stop().animate({'width':opts.peelSize,'height':opts.peelSize}, 200, "easeOutExpo");
				peelP_IMG.stop().animate({'width':opts.peelSize,'height':opts.peelSize}, 200, "easeOutExpo");
				lhover = true;
			}
		}
	},
	...
	updateCtrls = function(){
		...
		if(opts.peels){
			...
			peelN_IMG.attr('src',opts.peelNextIMG);
			peelP_IMG.attr('src',opts.peelPrevIMG);
			
			peels.width(opts.peelSize).height(opts.peelSize);
			
			if(opts.curr == 2 ){           
				peelP_IMG.attr('src',opts.peelPrevEndIMG);
			}
			if(opts.curr == opts.pTotal-4){
				peelN_IMG.attr('src',opts.peelNextEndIMG);
			}
		}
	},
...
$.fn.booklet.defaults = {
	...
	peels:              true,
	peelSize:           50,	
	peelNextIMG:        'images/peel-next.png',
	peelPrevIMG:        'images/peel-prev.png',	
	peelNextEndIMG:     'images/peel-next-end-white.png',
	peelPrevEndIMG:     'images/peel-prev-end-white.png',	

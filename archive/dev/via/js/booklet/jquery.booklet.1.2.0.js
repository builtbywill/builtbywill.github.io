...
$.fn.booklet = function(options){
	...
	var config = $.extend({}, $.fn.booklet.defaults, options), obj;
...
function booklet(target, options){
	...
	//tracking vars
	currhash = '', hash, i, j, p, diff, busy = false, init = false, rhover = false, lhover = false, h, a, playing = false,
	...
	addPages = function(html_1, title_1, chapter_1, html_2, title_2, chapter_2){
		var z = opts.pTotal;
		...
		chapters[z]   = chapter_1;
		chapters[z+1] = chapter_2;
		titles[z]     = title_1;
		titles[z+1]   = title_2;
		
		html_1 = $('<div class="b-page b-page-'+z+'"><div class="b-wrap b-wrap-left">'+html_1+'</div></div>').data('page',z);
		html_2 = $('<div class="b-page b-page-'+(z+1)+'"><div class="b-wrap b-wrap-right">'+html_2+'</div></div>').data('page',z+1);
		
		//give content the correct wrapper and page wrapper
		if(opts.closed && opts.covers){
			//html.find('.b-wrap').addClass('b-page-cover');
		}
		
		//add page numbers
		if(opts.pageNumbers && (!opts.closed || (opts.closed && !opts.covers))){
			html_1.find('.b-wrap').append('<div class="b-counter">'+(z+1)+'</div>');
			html_2.find('.b-wrap').append('<div class="b-counter">'+(z+2)+'</div>');
		}

	...
	$('.via-text-form').live('blur',function(){	
		...
		if(checkLength.isOver){
			...
			if($(this).hasClass('via-overflow')){
				...
				if(checkLength.lines.length - keepLines < endPageLine){
					endPageLine = checkLength.lines.length - keepLines;
				}
				...
				for(j=0;j<numNewPages;j++){
					...
					newPage.find('.via-text-form').attr('id',newPage.find('.via-text-form').attr('id').replace('0',newPageNum));
					newPage.find('.via-text-form').attr('name',newPage.find('.via-text-form').attr('name').replace('0',newPageNum));
